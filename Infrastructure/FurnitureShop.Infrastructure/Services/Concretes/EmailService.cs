    ...
    public async Task SendOrderConfirmationAsync(string toEmail, string toName, int orderId, decimal total, string lang)
    {
        var (subject, body) = lang switch
        {
            "ru" => ($"Заказ #{orderId} подтверждён — FurnitureShop",
                     $"<h2>Здравствуйте, {toName}!</h2><p>Ваш заказ <strong>#{orderId}</strong> успешно оформлен.</p><p>Итоговая сумма: <strong>{total:F2} AZN</strong></p>"),
            "en" => ($"Order #{orderId} Confirmed — FurnitureShop",
                     $"<h2>Hello, {toName}!</h2><p>Your order <strong>#{orderId}</strong> has been placed successfully.</p><p>Total: <strong>{total:F2} AZN</strong></p>"),
            _ => ($"#{orderId} nömrəli sifarişiniz qəbul edildi — FurnitureShop",
                     $"<h2>Salam, {toName}!</h2><p><strong>#{orderId}</strong> nömrəli sifarişiniz uğurla qəbul edildi.</p><p>Ümumi məbləğ: <strong>{total:F2} AZN</strong></p>")
        };
